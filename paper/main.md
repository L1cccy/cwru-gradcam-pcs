...
A supplementary analysis was conducted to identify the frequency regions where the trained model's Grad-CAM attention actually concentrates. Averaging saliency curves across 500 test samples from the CWRU dataset revealed that 74.0% of total saliency energy falls within the 2000–6000 Hz range, with a pronounced peak at approximately 2859 Hz (see Supplementary Material). Only 2.1% falls within the theoretical fault frequency bands (82–328 Hz), and 4.1% in the 0–500 Hz range excluding fault bands. A validation experiment using a randomly initialized (untrained) model produced a near-uniform saliency distribution with PCS = 3.68%, close to the theoretical random expectation of 3.3%—confirming that the trained model's concentrated attention pattern is learned behavior, not an artifact of the Grad-CAM implementation.

One possible interpretation of this finding is that bearing fault impulses excite structural resonances in the test rig [citation needed], and the CNN exploits the resulting resonance-modulated signal content in the 2–4 kHz range for classification. Under this interpretation, the model uses physically relevant information (vibration energy coupled through the sensor fixture and bearing housing), but this information is distinct from the fault characteristic frequencies measured by our PCS metric. This would explain the high accuracy–low PCS combination without invoking dataset-specific spurious correlations. However, this interpretation remains speculative without independent verification of the resonance frequencies of the CWRU test rig, and it cannot account for the complete failure of cross-dataset generalization (CWRU → MFPT: 4.3% accuracy).
